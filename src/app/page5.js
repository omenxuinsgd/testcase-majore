        ...
        <main className="flex-1 px-8 pt-8 md:pt-16 relative overflow-y-auto custom-scrollbar">
          <AnimatePresence mode="wait">
            <motion.div key={validIdx} initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0, rotateY: mousePos.x * 5, rotateX: -mousePos.y * 5 }} exit={{ opacity: 0, x: 30 }} transition={{ duration: 0.5 }} className="max-w-3xl mb-20" style={{ perspective: "1000px" }}>
              ...
              {/* PERBAIKAN: Kontainer Deskripsi ditingkatkan agar teks tidak terpotong dan kata tetap utuh */}
              <div className={`relative p-6 border-l-4 border-[#00ffff] backdrop-blur-sm mb-10 transition-all duration-500 h-auto min-h-[100px] flex flex-col ${isDarkMode ? 'bg-black/60 shadow-[0_0_30px_rgba(0,0,0,0.5)]' : 'bg-white/60'}`}>
                 ...
                 {/* PENTING:
                     1. break-normal: Mencegah pemotongan kata di tengah karakter (kata tetap utuh).
                     2. whitespace-normal: Memungkinkan pembungkusan teks alami.
                     3. overflow-visible: Memastikan teks yang memanjang ke bawah tetap terlihat.
                 */}
