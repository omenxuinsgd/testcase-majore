 ...
 * Kerangka luar terminal yang menangani header, footer, dan navigasi tab.
 * Navigasi Tab paralel slanted searah backslash (\ \).
 * =============================================================================
 */
const TerminalShell = ({ 
  data, 
  ...
  return (
    ...
    >
      ...
      <style jsx global>{`
        ...
        /* BREADCRUMB PATH ARROWS (RUNCING & TAJAM 18PX) */
        .path-arrow-start { 
...
const FingerprintModule = ({ data }) => {
  ...
  const handleKeyClick = (char) => {
    if (activeInput === 'nik') setNik(p => p + char);
    else if (activeInput === 'name') setUserName(p => p + char);
    else if (activeInput === 'identifier') setIdentifierKey(p => p + char);
  };

  const handleConnectToggle = () => {
    setIsConnected(!isConnected);
    setLogs(prev => [...prev, `[DEVICE] ${data?.short} ${!isConnected ? 'CONNECTED' : 'DISCONNECTED'} pada ${new Date().toLocaleTimeString()}`]);
  };
    ...
    <div className="flex-1 p-8 flex flex-col gap-4 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        <div className="w-full lg:w-[190px] flex flex-col items-center gap-2 shrink-0">
          <div className="relative w-full aspect-[1.25/1] border-2 border-[#00ffff]/20 bg-zinc-950 overflow-hidden group">
            <img src={isConnected ? "https://cdn.dribbble.com/userupload/23642809/file/original-129fd5d25fa96a3437877562aa243ad6.gif" : "https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExcm0zcDUyM2Yxazh2ZzJvN3NuaTkxMjh0NHU3djZqemYzbWFvZWp2OSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/5Akl9i8YiMbl5RFvGO/giphy.gif"} alt="Status" className={`w-full h-full object-cover transition-opacity duration-500 ${isConnected ? 'opacity-80' : 'opacity-40'}`} />
            <div className="absolute inset-0 flex flex-col items-end justify-start text-right p-4 pointer-events-none">
              <span className="text-[9px] text-zinc-400 font-black uppercase tracking-widest block leading-tight">Device Is</span>
              ...
              <button onClick={handleConnectToggle} className={`pointer-events-auto mt-4 px-4 py-1.5 border-2 text-[9px] font-black uppercase active:scale-95 shadow-lg ${isConnected ? 'border-red-500 text-red-500 hover:bg-red-500 hover:text-white' : 'border-[#00ffff] text-[#00ffff] hover:bg-[#00ffff] hover:text-black'}`}>
        ...
        <div className="flex-1 h-[155px] border-2 border-[#00ffff]/40 bg-zinc-900/60 pt-6 px-6 pb-12 relative rounded-sm group flex flex-col justify-start gap-4 overflow-visible">
          ...
          <div className="grid grid-cols-2 gap-6 text-left">
            <div className="space-y-1.5"><label className="text-[9px] text-[#00ffff] font-black uppercase tracking-widest">Select Mode</label><select className="w-full bg-black border-2 border-[#00ffff]/20 text-[10px] p-2 text-white outline-none focus:border-[#00ffff] font-pixel cursor-pointer" onChange={(e)=>{}}><option>Standard_Credential_ID</option></select></div>
            <div className="space-y-1.5"><label className="text-[9px] text-[#00ffff] font-black uppercase tracking-widest">Identifier Key</label><input value={identifierKey} onFocus={() => setActiveInput('identifier')} onChange={(e) => setIdentifierKey(e.target.value)} placeholder="Enter Serial / NIK..." className={`w-full bg-black border-2 transition-all text-[10px] p-2 text-[#00ffff] outline-none font-mono ${activeInput === 'identifier' ? 'border-[#00ffff]' : 'border-[#00ffff]/20'}`} /></div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 h-fit">
        <div className="border-2 border-[#00ffff]/20 bg-zinc-950/80 p-4 flex flex-col rounded-sm relative text-left">
          <div className="flex justify-between items-center border-b border-[#00ffff]/10 pb-1 mb-2 font-black text-[#00ffff]/60 uppercase text-[9px] tracking-widest"><span>Registry_Manual</span><Keyboard size={12} className="text-[#00ffff]/30" /></div>
          <div className="flex flex-col gap-3 font-pixel">
            <div className={`flex flex-col gap-1 border-b-2 transition-all duration-300 ${activeInput === 'nik' ? 'border-[#00ffff]' : 'border-[#00ffff]/20'}`}><input value={nik} onFocus={() => setActiveInput('nik')} onChange={(e) => setNik(e.target.value)} placeholder="Enter Your NIK" className="w-full bg-transparent text-[10px] p-1 text-[#00ffff] outline-none" /></div>
            <div className={`flex flex-col gap-1 border-b-2 transition-all duration-300 ${activeInput === 'name' ? 'border-[#00ffff]' : 'border-[#00ffff]/20'}`}><input value={userName} onFocus={() => setActiveInput('name')} onChange={(e) => setUserName(e.target.value)} placeholder="Enter Your Name" className="w-full bg-transparent text-[10px] p-1 text-[#00ffff] outline-none" /></div>
          </div>
          <div className="grid grid-cols-10 gap-1 mt-3 select-none">{[...Array(30)].map((_, i) => <div key={i} onClick={() => handleKeyClick(String.fromCharCode(65 + i))} className="aspect-square border border-[#00ffff]/30 bg-[#00ffff]/5 text-[8px] flex items-center justify-center text-[#00ffff] hover:bg-[#00ffff] cursor-pointer active:scale-90 transition-all">{String.fromCharCode(65 + i)}</div>)}</div>
          <div className="flex gap-2 mt-3"><button className="flex-1 py-1.5 border-2 border-[#00ffff] text-[#00ffff] text-[9px] font-black hover:bg-[#00ffff] hover:text-black uppercase active:scale-95 transition-all">Submit</button><button onClick={() => {setNik(""); setUserName("");}} className="flex-1 py-1.5 border-2 border-[#ff00ff] text-[#ff00ff] text-[9px] font-black hover:bg-[#ff00ff] hover:text-white uppercase active:scale-95 transition-all">Cancel</button></div>
        </div>
        <div className="border-2 border-[#00ffff]/20 bg-black/90 p-4 flex flex-col rounded-sm text-left text-zinc-400 font-mono text-[10px] overflow-hidden shadow-inner">
          <div className="flex items-center gap-2 text-[#00ffff] mb-2 uppercase font-black tracking-widest border-b border-[#00ffff]/10 pb-1"><Activity size={10} className="animate-pulse" /><span>Console_Buffer</span></div>
          <div className="flex-1 overflow-hidden space-y-1">{logs.slice(-5).map((log, idx) => (<div key={idx} className="flex gap-2 leading-none"><span className="opacity-30">{(idx+1).toString().padStart(2,'0')}</span><span>{log}</span></div>))}<div className="animate-pulse text-[#00ffff] font-black">_</div></div>
        </div>
      </div>
    </div>
  );
...
const BarcodeScannerModule = ({ data }) => {
  ...
  const handleConnectToggle = () => { setIsConnected(!isConnected); setLogs(p => [...p, `[SCANNER] ${!isConnected ? 'ONLINE' : 'OFFLINE'} pada ${new Date().toLocaleTimeString()}`]); };
  ...
  return (
    <div className="flex-1 p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col lg:flex-row gap-10 items-start shrink-0">
        <div className="w-full lg:w-[190px] flex flex-col items-center gap-2 shrink-0">
          <div className="relative w-full aspect-[1.25/1] border-2 border-[#00ffff]/20 bg-zinc-950 overflow-hidden group">
            <img src={isConnected ? "https://cdn.dribbble.com/userupload/11267440/file/original-4d437e61e0e8e604f8e6e58b1a3782b7.gif" : "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExM3NueG50MGJ2ZWZ5dWNxZ3R6ZTF3ZTN1Nnd3NTh3ZTN1Nnd3NTh3ZSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/vFhIOWC9S4GzM7T0yC/giphy.gif"} alt="Scanner" className={`w-full h-full object-cover transition-opacity duration-500 ${isConnected ? 'opacity-80' : 'opacity-40'}`} />
            <div className="absolute inset-0 flex flex-col items-end justify-start text-right p-4 pointer-events-none">
              ...
              <button onClick={handleConnectToggle} className="pointer-events-auto mt-4 px-4 py-1.5 border-2 text-[9px] font-black uppercase active:scale-95 shadow-lg border-[#00ffff] text-[#00ffff] hover:bg-[#00ffff] hover:text-black transition-all">
                {isConnected ? 'Disconnect' : 'Connect'}
              </button>
            </div>
          </div>
        ...
        <div className="flex-1 h-[155px] border-2 border-[#00ffff]/40 bg-zinc-900/60 pt-6 px-6 pb-12 relative rounded-sm group flex flex-col justify-start gap-4 overflow-visible">
          ...
          <div className="flex gap-6 items-end z-0">
            <div className="flex-1 space-y-1.5"><label className="text-[9px] text-[#00ffff] font-black block uppercase tracking-widest">Select Mode</label><select value={scanMode} onChange={(e) => setScanMode(e.target.value)} className="w-full bg-black border-2 border-[#00ffff]/20 text-[10px] p-2 text-white outline-none focus:border-[#00ffff] font-pixel cursor-pointer transition-colors"><option value="Camera">Camera</option><option value="Upload_File">Upload_File</option></select></div>
            {scanMode === "Upload_File" && (<div className="flex-1 space-y-1.5"><button onClick={() => fileInputRef.current.click()} className="w-full bg-[#00ffff]/10 border-2 border-dashed border-[#00ffff]/40 text-[10px] p-2 text-[#00ffff] font-black flex items-center justify-center gap-2 transition-all"><Folder size={12} /><span>Browse_PC</span></button><input type="file" ref={fileInputRef} className="hidden" /></div>)}
          </div>
          <div className="flex gap-4 mt-2"><button className="flex-1 py-1.5 bg-zinc-950 border-2 border-[#00ffff] text-[#00ffff] text-[10px] font-black uppercase active:scale-95 shadow-md transition-all">Start Scan</button></div>
        </div>
      </div>
      <div className="flex-1 border-2 border-[#00ffff]/20 bg-black/90 p-5 flex flex-col rounded-sm overflow-hidden text-left font-mono text-[11px] text-zinc-400 shadow-inner"><div className="flex items-center gap-3 text-[#00ffff] mb-4 uppercase font-black border-b border-[#00ffff]/10 pb-2"><Activity size={12} className="animate-pulse" /><span>Console_Output</span></div><div className="flex-1 overflow-y-auto custom-scrollbar">{logs.map((l, i) => (<div key={i}>{l}</div>))}</div></div>
    </div>
  );
};
...
/**
 * =============================================================================
 * 5. COMPONENT: OCRScannerModule (UPDATED LAYOUT)
 * =============================================================================
 */
const OCRScannerModule = ({ data, logs, setLogs }) => {
  const [language, setLanguage] = useState("English");
  const [psm, setPsm] = useState("3 - Auto (Default)");
  const [oem, setOem] = useState("3 - LSTM + Legacy (Paling Akurat)");
  const [isConnected, setIsConnected] = useState(false);
  const fileInputRef = useRef(null);
  ...
  const handleConnectToggle = () => { 
    ...
    setLogs(p => [...p, `[OCR_SYSTEM] ${nextStatus ? 'CONNECTED' : 'DISCONNECTED'} pada ${new Date().toLocaleTimeString()}`]); 
  };
  ...
  return (
    <div className="flex-1 p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
      <div className="flex flex-col lg:flex-row gap-8 items-start shrink-0">
        ...
        <div className="w-full lg:w-[190px] flex flex-col items-center gap-2 shrink-0">
          <div className="relative w-full aspect-[1.25/1] border-2 border-[#00ffff]/20 bg-zinc-950 overflow-hidden group">
            <img src={isConnected ? "https://cdn.dribbble.com/userupload/11132644/file/original-b9c1d6365f57c617e4e1352a9e3f3a8b.gif" : "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExM3NueG50MGJ2ZWZ5dWNxZ3R6ZTF3ZTN1Nnd3NTh3ZTN1Nnd3NTh3ZSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/5Akl9i8YiMbl5RFvGO/giphy.gif"} alt="OCR Status" className={`w-full h-full object-cover transition-opacity duration-500 ${isConnected ? 'opacity-80' : 'opacity-40'}`} />
            <div className="absolute inset-0 flex flex-col items-end justify-start text-right p-4 pointer-events-none">
              <span className={`text-[12px] font-black uppercase tracking-tighter mt-1 animate-pulse ${isConnected ? 'text-emerald-400' : 'text-red-500'}`}>{isConnected ? 'Active' : 'Offline'}</span>
              <button onClick={handleConnectToggle} className="pointer-events-auto mt-4 px-4 py-1.5 border-2 text-[9px] font-black uppercase active:scale-95 shadow-lg border-[#00ffff] text-[#00ffff] hover:bg-[#00ffff] hover:text-black transition-all">
                {isConnected ? 'Disconnect' : 'Connect'}
          ...
          <span className="text-[8px] text-[#00ffff]/40 font-bold uppercase tracking-[0.3em]">OCR_Recognition_V.2</span>
        </div>

        {/* OCR PARAMETERS AREA */}
        <div className="flex-1 border-2 border-[#00ffff]/40 bg-zinc-900/60 p-6 relative rounded-sm flex flex-col gap-4 overflow-visible shadow-2xl">
          <div className="absolute -top-[12px] left-6 bg-white text-black px-4 py-0.5 text-[10px] font-black uppercase z-[50]">OCR Capture & Accuracy</div>
          
          <div className="grid grid-cols-2 gap-8">
             {/* 1. Upload Gambar */}
             <div className="space-y-4">
                <div className="space-y-1.5">
                   <label className="text-[9px] text-[#00ffff]/60 font-black block uppercase flex items-center gap-2 tracking-widest">
                     <ImageIcon size={10} /> 1. Upload Gambar:
                   </label>
                   <div className="flex gap-2">
                      <button 
                        onClick={() => fileInputRef.current.click()} 
                        className="flex-1 bg-[#00ffff]/10 border-2 border-dashed border-[#00ffff]/40 text-[10px] p-2 text-[#00ffff] font-black hover:bg-[#00ffff]/20 hover:border-[#00ffff] active:scale-90 transition-all flex items-center justify-center gap-2"
                      >
                         <Folder size={12} />
                         <span>Choose_File_or_Folder</span>
                      </button>
                      <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => {
                        const file = e.target.files[0];
                        if (file) setLogs(p => [...p, `[FILE] Loaded: ${file.name}`]);
                      }} />
                   </div>
                </div>
             </div>
             ...
             <div className="space-y-3">
                <label className="text-[9px] text-[#00ffff]/60 font-black block uppercase flex items-center gap-2 mb-1 tracking-widest">
                   <Settings2 size={10} /> 2. Parameter Akurasi:
                </label>
                <div className="grid grid-cols-1 gap-2">
                   <div className="flex items-center gap-3">
                     <span className="text-[9px] text-white w-24 shrink-0 font-bold uppercase tracking-tighter">Bahasa:</span>
                     <select value={language} onChange={(e) => setLanguage(e.target.value)} className="flex-1 bg-black border-2 border-[#00ffff]/10 text-[9px] p-1.5 text-[#00ffff] outline-none focus:border-[#00ffff] cursor-pointer transition-colors">
                   ...
                   <div className="flex items-center gap-3">
                     <span className="text-[9px] text-white w-24 shrink-0 font-bold uppercase tracking-tighter">PSM:</span>
                     <select value={psm} onChange={(e) => setPsm(e.target.value)} className="flex-1 bg-black border-2 border-[#00ffff]/10 text-[9px] p-1.5 text-[#00ffff] outline-none focus:border-[#00ffff] cursor-pointer transition-colors">
                       <option>3 - Auto (Default)</option>
                       <option>1 - Automatic Page Segment</option>
                       <option>6 - Single Uniform Block</option>
                       <option>7 - Single Text Line</option>
                     </select>
                   </div>
                   <div className="flex items-center gap-3">
                     <span className="text-[9px] text-white w-24 shrink-0 font-bold uppercase tracking-tighter">Engine AI:</span>
                     <select value={oem} onChange={(e) => setOem(e.target.value)} className="flex-1 bg-black border-2 border-[#00ffff]/10 text-[9px] p-1.5 text-[#00ffff] outline-none focus:border-[#00ffff] cursor-pointer transition-colors">
                       <option>3 - LSTM + Legacy (Paling Akurat)</option>
                       <option>1 - LSTM Only</option>
                       <option>0 - Legacy Only</option>
                     </select>
                   </div>
                </div>
             </div>
          ...
          <div className="mt-2">
             <button onClick={() => setLogs(p => [...p, `[ACTION] Ekstraksi teks dimulai...` || ''])} className="w-full py-2.5 bg-[#00ffff] text-black text-[11px] font-black uppercase tracking-[0.3em] shadow-[0_0_20px_rgba(0,255,255,0.4)] hover:brightness-125 transition-all active:scale-[0.98]">
      ...
      {/* HASIL RECOGNITION RESULT (Tukar Posisi ke Kanan Bawah) */}
      <div className="flex-1 flex flex-col min-h-[220px]">
        <div className="flex-1 border-2 border-[#00ffff]/40 bg-zinc-900/60 p-5 rounded-sm relative overflow-hidden flex flex-col gap-4 shadow-[0_0_20px_rgba(0,255,255,0.05)]">
           ...
           <div className="flex justify-between items-center border-b border-[#00ffff]/20 pb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-[#00ffff] uppercase tracking-[0.2em]">OCR Recognition Result</span>
              </div>
              <ClipboardType size={14} className="text-[#00ffff]/60" />
           </div>

           <div className="flex-1 bg-black/80 border border-[#00ffff]/10 p-6 flex flex-col justify-center items-center rounded-sm group relative">
           ...
           <div className="flex justify-between items-center text-[8px] font-black uppercase">
              <span className="text-zinc-500 tracking-widest">Confidence: 100%</span>
    ...
    <div className="w-[500px] flex flex-col items-start shrink-0">
      ...
      {!isPrinter && (
        <>
          ...
          {/* AREA DI BAWAH PATH - SWAP LOGIC */}
          {isOCR ? (
            /* OCR_PROCESS_LOGS SEKARANG DI SIDEBAR KIRI */
            ...
            /* CARD HASIL PEMBACAAN (TETAP DI SIDEBAR UNTUK BARCODE) */
            <div className="w-full border-2 border-[#00ffff]/40 bg-zinc-900/60 p-5 rounded-sm relative overflow-hidden flex flex-col gap-4 shadow-[0_0_20px_rgba(0,255,255,0.05)] h-fit">
               ...
               <div className="flex justify-between items-center border-b border-[#00ffff]/20 pb-2">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-[#00ffff] animate-pulse rounded-full" />
                    <span className="text-[10px] font-black text-[#00ffff] uppercase tracking-[0.2em]">Scanner Decode Result</span>
                  </div>
               ...
               <div className="flex-1 bg-black/80 border border-[#00ffff]/10 p-6 flex flex-col justify-center items-center min-h-[120px] rounded-sm group relative">
                  <div className="text-center w-full">
                    <span className="text-[8px] text-zinc-500 uppercase block mb-3 tracking-[0.3em] font-black italic opacity-50">[ SYSTEM_READY ]</span>
                    <div className="px-4 py-2 border border-[#00ffff]/5 bg-[#00ffff]/5 inline-block min-w-[150px]">
                      <h4 className="text-[18px] font-mono text-[#00ffff] break-all uppercase leading-none tracking-tighter drop-shadow-[0_0_8px_rgba(0,255,255,0.4)]">{"--- EMPTY ---"}</h4>
                    </div>
                  </div>
               </div>
